class Admin::PromosController < Admin::AdminController
  ...
  def index
    ...
    if params[:order_by].present?
      ...
      @promos = @promos.order('%s %s' % [@current_sort_column, @sort_dir])
    end
    @promos = @promos.page(params[:page]).per(20)
  end
  ...
  def destroy
    @promo.destroy
    respond_to do |format|
      format.html { redirect_to admin_promos_path, notice: 'Promo was successfully destroyed.' }
  ...
  def promo_params
    params.require(:promo).permit(:name, :course_id, :stripe_coupon, :enabled, :expires_on, :message)
  end
end
